...
def create_network(N = 1000, g = 1.5, alpha = 1, p = 0.1, neurons_recorded = 10):
    ...
    network = pd.Series({
        ...
        'alpha' : alpha,
        'wo' : g*scale*np.random.normal(0, 1/N/p, (N//2, 1)),
        'J_FG' : g*scale*np.random.normal(0, 1/N/p, (N//2, 1)), 	# synaptic strengths from internal pool to control unit
...
def train(network, time, learn_every, ft, plot_training = True):
    ...
    for t in simtime:
        ...
        network.x = (1.0-dt)*network.x + network.J_GG*(network.r*dt) + network.J_GF*(network.y*dt); # note the y here.
...
network = test(network, simtime, ft/5)
plot_activity(network, simtime, title = 'Spontaneous Activity')

network_trained = train(network, simtime, 3, ft)
plot_activity(network_trained, simtime, title = 'Learning')

network_tested = test(network_trained, simtime, ft)
plot_activity(network_tested, simtime, title = 'Post Learning')
